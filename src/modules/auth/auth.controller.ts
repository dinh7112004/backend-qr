...
export class MeController {
  ...
  @Post('profile')
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Cập nhật profile', description: 'Cập nhật tên và số điện thoại cho thiết bị hiện tại.' })
  async updateProfile(
    @Headers('x-device-id') deviceId: string,
    ...
    if (existingUser) {
      // If we found an existing user with this phone, merge current guest data into it
      // and update its display name
      existingUser.displayName = body.displayName;
      existingUser.deviceId = deviceId; // Link this device to the existing account

      // Merge loyalty points if current guest has some
      ...
      // If the existing user is NOT the same as currentUser (different IDs), delete the guest one
